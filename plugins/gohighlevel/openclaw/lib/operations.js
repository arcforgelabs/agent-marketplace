...
export async function locationStatus(client, config, _params) {
    const location = await client.request("GET", `/locations/${config.locationId}`);
    const record = location.location ?? location;
    const named = record;
    return {
        connected: true,
...
export async function contacts(client, config, params) {
            ...
            if (params.filters) {
                return client.request("POST", "/contacts/search", {
                    body: compact({
                        locationId: config.locationId,
                        page: params.page ?? 1,
                        pageLimit: capLimit(params.limit),
                        query,
...
export async function tasks(client, _config, params) {
    ...
    switch (action) {
        ...
        case "complete":
            return client.request("PUT", `/contacts/${contactId}/tasks/${requireString(params, "taskId", action)}/completed`, { body: { completed: params.completed ?? true } });
        case "delete":
...
export async function conversations(client, config, params) {
...
export async function messages(client, config, params) {
    ...
    switch (action) {
        ...
        case "recording":
            return client.request("GET", `/conversations/messages/${requireString(params, "messageId", action)}/locations/${config.locationId}/recording`);
        case "transcription":
            return client.request("GET", `/conversations/locations/${config.locationId}/messages/${requireString(params, "messageId", action)}/transcription`);
        default:
            throw new Error(`Unsupported messages action: ${action}`);
    }
}
export async function opportunities(client, config, params) {
...
export async function pipelines(client, config, params) {
    ...
    switch (action) {
        ...
        case "create": {
            ...
            if (!names.length)
                throw new Error("stages is required for pipeline create.");
...
export async function customFields(client, config, params) {
...
export async function calendars(client, config, params) {
    ...
    switch (action) {
        ...
        case "get_appointment":
            return client.request("GET", `/calendars/events/appointments/${requireString(params, "appointmentId", action)}`);
        ...
        case "update_appointment":
            return client.request("PUT", `/calendars/events/appointments/${requireString(params, "appointmentId", action)}`, {
                body: compact({
                    calendarId: asString(params.calendarId),
                    contactId: asString(params.contactId),
                    startTime: asString(params.startTime),
...
export async function workflows(client, config, params) {
    ...
    switch (action) {
        ...
        case "enroll":
            return client.request("POST", `/contacts/${requireString(params, "contactId", action)}/workflow/${requireString(params, "workflowId", action)}`);
        case "unenroll":
            return client.request("DELETE", `/contacts/${requireString(params, "contactId", action)}/workflow/${requireString(params, "workflowId", action)}`);
        default:
            throw new Error(`Unsupported workflows action: ${action}`);
    }
}
export async function users(client, config, params) {
...
export async function tags(client, config, params) {
    const action = requireString(params, "action", "ghl_tags");
    if (action !== "list")
        throw new Error(`Unsupported tags action: ${action}`);
