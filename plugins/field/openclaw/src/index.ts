...
import { FieldPluginError, mcpServerConfig, resolveOrigin, type FieldPluginConfig } from "./origin.js";
...
export default defineToolPlugin({
  ...
  description:
    "Connect OpenClaw to a customer's Field instance with OAuth. Quote and catalog tools come from Field MCP after login.",
  configSchema,
  tools: (tool) => [
    tool({
      ...
      async execute(_params, config: FieldPluginConfig) {
        if (!config?.origin) {
          throw new FieldPluginError(
            "Set plugins.entries.arcforgelabs-field.config.origin to the customer's Field HTTPS origin",
          );
        }
