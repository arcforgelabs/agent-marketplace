...
async function main() {
  ...
  if (!group || group === "--help") {
    ...
    console.log(
      "Fergus vendor limit: 100 requests per minute per company, shared across tokens and endpoints.",
    );
    console.log(
      "This CLI uses a process-wide local governor (default 80/min, hard-capped at 100). Do not parallel-spray list calls.",
    );
    console.log(
      "Environment: FERGUS_API_TOKEN (required), optional FERGUS_COMPANY_ID, FERGUS_MAX_REQUESTS_PER_MINUTE, FERGUS_TIMEZONE.",
    );
  ...
  const data = await groups[group as keyof typeof groups](client, config, params);
