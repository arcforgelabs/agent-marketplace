...
- **Deputy / OpenClaw:** canonical source and staging tooling live in `arcforgelabs/arc-forge-tools`. Follow its `docs/architecture/plugin-publishing.md` and `forge-tools plugin stage deputy` entry point. The package lives at `plugins/deputy/openclaw/` and records source provenance. Runtime id `arcforgelabs-deputy`. Permanent REST token via SecretRef plus tenant `installHost`; no community MCP wrap and no OAuth/PKCE.
- **Fergus / OpenClaw:** canonical source and staging tooling live in `arcforgelabs/arc-forge-tools`. Follow its `docs/architecture/plugin-publishing.md` and `forge-tools plugin stage fergus` entry point. The package lives at `plugins/fergus/openclaw/` and records source provenance. Runtime id `arcforgelabs-fergus`. REST PAT via SecretRef; no community MCP wrap.
